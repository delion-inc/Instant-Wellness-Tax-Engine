...
import { Copy01Icon, MapPinIcon } from "@hugeicons/core-free-icons";
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex flex-col gap-6 px-4 pb-6">
          {/* Map placeholder */}
          <div className="bg-muted/50 flex h-32 flex-col items-center justify-center gap-2 rounded-lg border border-dashed">
            <HugeiconsIcon
              icon={MapPinIcon}
              strokeWidth={2}
          ...
          <div>
            ...
            <div className="rounded-lg border p-3">
              ...
              <DetailRow
                ...
                value={
                  order.timestamp
                    ? format(new Date(order.timestamp), "MMM d, yyyy HH:mm:ss")
                    : "-"
                }
              />
              <DetailRow
          ...
          <div>
            ...
            <div className="rounded-lg border p-3">
              <DetailRow
                ...
                value={formatRate(order.compositeTaxRate)}
              />
              <DetailRow
                label="Tax amount"
              ...
              <DetailRow
                label="State rate"
                value={formatRate(order.stateRate)}
              />
              <DetailRow
                label="County rate"
                value={formatRate(order.countyRate)}
              />
              <DetailRow
                label="City rate"
                value={formatRate(order.cityRate)}
              />
          ...
          {jurisdictions && (
            <div>
              ...
              <div className="rounded-lg border p-3">
                {jurisdictions.state && (
                  <DetailRow label="State" value={jurisdictions.state} />
                )}
                {jurisdictions.county && (
                  <DetailRow label="County" value={jurisdictions.county} />
                )}
                {jurisdictions.city && (
                  <DetailRow label="City" value={jurisdictions.city} />
                )}
