...
const hasTaxBreakdown = (data: OrderResponse) =>
  data.status === "CALCULATED" || data.status === "ADDED";

export const ManualOrderResult = forwardRef<HTMLDivElement, ManualOrderResultProps>(
  function ManualOrderResult({ data, error, isPending, isIdle }, ref) {
    if (isIdle) {
      return (
        <Card ref={ref} className="flex h-full flex-col">
    ...
    const specialRates = data.specialRates ?? [];
        ...
        <CardContent className="flex-1 space-y-4">
          ...
          {hasTaxBreakdown(data) ? (
            ...
            <>
              <Separator />
              <p className="text-muted-foreground text-sm">
                The provided location is outside the supported tax jurisdictions. No tax rates were
                applied.
              </p>
              <div className="rounded-lg border bg-muted/40 px-3 py-2">
                <ResultRow label="Subtotal" value={formatCurrency(data.subtotal)} />
